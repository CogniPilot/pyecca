from pyecca2.rotation import Dcm
from pyecca2.rotation import Quat
import casadi as ca
import pytest
...
def test_to_from_dcm():
    ...
    assert ca.norm_fro(Dcm.from_quat(q_check) - dcm_check) < tol


def test_product():
    print(type(a * b))
    assert ca.norm_fro((a * b).to_dcm() - ca.mtimes(Dcm.from_quat(a), Dcm.from_quat(b)) < tol
